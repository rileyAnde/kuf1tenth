...
class F1TenthEnv(Node):
    ...
    def pose_callback(self, data):
        ...
        if self.check_collision(data.pose):
            self.done = True
            self.get_logger().warn("Collision detected, resetting...")

    def check_collision(self, pose):
        car_x, car_y = pose.position.x, pose.position.y
        track_bounds = [(-10, -10), (10, 10)]  # Example bounds; replace with real track data
        collision = not (track_bounds[0][0] <= car_x <= track_bounds[1][0] and 
                         track_bounds[0][1] <= car_y <= track_bounds[1][1])
        if collision:
            self.get_logger().warn(f"Collision at position: ({car_x}, {car_y})")
        return collision
